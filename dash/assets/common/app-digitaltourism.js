const pathApi = "../assets/json/data_digitaltourism.json";
...
async function fetchData() {
  try {
    ...
    sentimentAnalysis(percentPositive, percentNeutral, percentNegative);
...
function topFive() {
  ...
  fiveItems.forEach((item, index) => {
    ...
    for (let i = 1; i <= index + 1; i++) {
      topFiveHTML += `
      <img src="../assets/img/avatars/digitaltourism/top-${i}.jpg" alt="Credit Card" class="img-profile" />
      `;
    }
    topFiveHTML += `</div>`;
    topFiveHTML += `
...
function countPositive() {
  ...
  data.forEach((item) => {
    item.sentiment.forEach((sentimentItem) => {
...
function sentimentAnalysis(percentPositive, percentNeutral, percentNegative) {
  const showSentimentAnalysis = document.getElementById("show-analysis");
  let dataAnalysis = "";
  if (percentNegative !== 0) {
    dataAnalysis += `
    <div class="progress-bar bg-nagative shadow-none" role="progressbar" style="width: ${percentNegative}%" aria-valuenow="${percentNegative}" aria-valuemin="0" aria-valuemax="100">${percentNegative}%</div>
    `;
  } else {
    dataAnalysis += ``;
  }
  ...
  if (percentNeutral !== 0) {
    dataAnalysis += `
    <div class="progress-bar bg-neutral shadow-none" role="progressbar" style="width: ${percentNeutral}%" aria-valuenow="${percentNeutral}" aria-valuemin="0" aria-valuemax="100">${percentNeutral}%</div>
    `;
  } else {
    dataAnalysis += ``;
  }
  ...
  if (percentPositive !== 0) {
    dataAnalysis += `
    <div class="progress-bar bg-positive shadow-none" role="progressbar" style="width: ${percentPositive}%" aria-valuenow="${percentPositive}" aria-valuemin="0" aria-valuemax="100">${percentPositive}%</div>
    `;
  } else {
    dataAnalysis += ``;
  }
